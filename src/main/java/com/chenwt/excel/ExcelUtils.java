 ...
 * @campany：zkzj
 * @author: chenwt
 * @date:2019-08-05 15:21
 * @description: excel操作公共类-提供excel按照模板输出
 */
public class ExcelUtils {
     ...
     * @param sheetDataArr 模板数据
     */

    public static void writeData(String model , OutputStream out, SheetData... sheetDataArr ) {
        Workbook wb = null;
        try {
        ...
        //就一个的话 直接用模板
        int size = sheetDataArr.length ;
        for(int i = 0 ; i < size  ; i++) {
            if(i == 0) {
                wb.setSheetName(0, sheetDataArr[0].getName());
            } else {
                Sheet toSheet = wb.createSheet(sheetDataArr[i].getName());
                //复制格式
        ...
        for(int i = 0 ; i < size  ; i++) {
            //写数据
            writeData(sheetDataArr[i], wb.getSheetAt(i));
        }
    ...
    static void writeData(SheetData sheetData , String keyWind , Cell cell , Sheet sheet) {
        ...
        if(keyWind.startsWith("#")) {
            ...
            List<? extends Object> listdata = sheetData.getDataList();
